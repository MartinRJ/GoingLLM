...
def response_task(usertask, task_id, dogoogleoverride):
    ...
    if not dogoogleoverride:
        if calculate_available_tokens(MAX_TOKENS_DECISION_TO_GOOGLE) < 1:
            ...
            prompt = "Es wurde folgende Anfrage gestellt: >>" + usertask + "<<. Benötigst du weitere Informationen aus einer Google-Suche, um diese Anfrage zu erfüllen? Bitte antworte mit \"Ja.\" oder \"Nein.\". Falls du keinen Zugriff auf die benötigten Informationen hast um die Anfrage zu beantworten (zum Beispiel falls du nach Dingen wie der aktuellen Uhrzeit oder nach aktuellen Ereignissen gefragt wirst), so antworte mit \"Ja.\"."
            system_prompt = "Ich bin dein persönlicher Assistent für die Internetrecherche und antworte ausschließlich nur mit \"Ja.\" oder \"Nein.\". Mir ist bewusst, dass ich zur Lösung der Aufgabe/Anfrage im Verlauf des Chats bei Bedarf mit neuen relevanten Google-Suchresultaten gespeist werde. Ich antworte immer mit \"Ja.\" für den Fall, dass ich keinen Zugriff auf erfragte Informationen habe (zum Beispiel falls nach Dingen wie der aktuellen Uhrzeit oder nach aktuellen Ereignissen gefragt wird), in dem Wissen, dass mir diese Informationen im Verlauf des Chats noch zur Verfügung gestellt werden."
