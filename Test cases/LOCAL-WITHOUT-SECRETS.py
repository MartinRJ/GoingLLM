...
SECRET_KEY = "" #--------------------------------------------------------------------------------------------- SECRETS NEED TO BE FILLED IN HERE ! --------------------
CUSTOMSEARCH_KEY = "" #--------------------------------------------------------------------------------------------- SECRETS NEED TO BE FILLED IN HERE ! --------------------
CX = "" #--------------------------------------------------------------------------------------------- SECRETS NEED TO BE FILLED IN HERE ! --------------------
TEMPERATURE_DECISION_TO_GOOGLE = 0.2
MAX_TOKENS_DECISION_TO_GOOGLE = 3
TEMPERATURE_CREATE_SEARCHTERMS = 0.36
MAX_TOKENS_CREATE_SEARCHTERMS = 400
TEMPERATURE_SUMMARIZE_RESULT = 0.2
MAX_TOKENS_SUMMARIZE_RESULT = 300
MIN_TOKENS_SUMMARIZE_RESULT = 115
MAX_FILE_CONTENT = 8000
NUMBER_GOOGLE_RESULTS = 3
NUMBER_OF_KEYWORDS = 3
TEMPERATURE_FINAL_RESULT = 0.3
MAX_TOKENS_FINAL_RESULT = 2380
TEMPERATURE_SELECT_SEARCHES = 0.4
MAX_TOKENS_SELECT_SEARCHES_LENGTH = 256
TEMPERATURE_MORE_SEARCHES = 0.3
MAX_TOKENS_MORE_SEARCHES_LENGTH = 1300
TEMPERATURE_CLEANUP = 0.3
MAX_TOKENS_CLEANUP_LENGTH = 50
BODY_MAX_LENGTH = 13500
GLOBAL_CHATCOMPLETION_TIMEOUT = 42
...
AUTH_UNAME = "" #--------------------------------------------------------------------------------------------- SECRETS NEED TO BE FILLED IN HERE ! --------------------
AUTH_PASS = "" #--------------------------------------------------------------------------------------------- SECRETS NEED TO BE FILLED IN HERE ! --------------------
            ...
            if searchresults == None or not searchresults:
                ...
                if moresearches and valid_moresearches and not detectNo(moresearches):
                    ...
                    if more_searchresults:
                        searchresults = more_searchresults
...
def process_more_searchresults_response(json_object, searchresults, usertask, task_id):
    ...
    try:
        string = ''.join([f'{summarytext_newline if i > 0 else ""}{summarytext_entry_start}{searchresults[i][str(i)]["summary"]}{summarytext_entry_end_URL_start}{searchresults[i][str(i)]["URL"]}{summarytext_entry_URL_end}' for i in range(len(searchresults)) if len(searchresults[i][str(i)]["summary"]) > 0])
    ...
    if "search" in actions:
        ...
        if keywords:
            ...
            if new_searchresults == None or not new_searchresults:
                ...
                # Flatten searchresults and new_searchresults to a list of dictionaries
                flat_searchresults = [entry for sublist in searchresults for entry in sublist.values()]
                flat_new_searchresults = [entry for sublist in new_searchresults for entry in sublist.values()]

                # Extend searchresults with new_searchresults
                flat_searchresults.extend(flat_new_searchresults)

                # Reindex the combined list of dictionaries
                reindexed_searchresults = [{str(i): result} for i, result in enumerate(flat_searchresults)]
...
def generate_final_response_with_search_results(searchresults, usertask, task_id):
    try:
        finalquery = ''.join([f'{summarytext_newline if i > 0 else ""}{summarytext_entry_start}{searchresults[i][str(i)]["summary"]}{summarytext_entry_end_URL_start}{searchresults[i][str(i)]["URL"]}{summarytext_entry_URL_end}' for i in range(len(searchresults)) if len(searchresults[i][str(i)]["summary"]) > 0])
    except TypeError as e:
    ...
    if not finalquery.endswith(summarytext_entry_URL_end):
        finalquery = truncate_at_last_period_or_newline(finalquery) # make sure the last summary also ends with period or newline, if it was cut, and therefore doesn't end with summarytext_entry_URL_end as expected.
...
def do_download_and_summary(weighting, gpturls, keyword, usertask, searchresults, URL, task_id):
    ...
    test_finalquery = ''.join([f'{summarytext_newline if i > 0 else ""}{summarytext_entry_start}{searchresults[i][str(i)]["summary"]}{summarytext_entry_end_URL_start}{searchresults[i][str(i)]["URL"]}{summarytext_entry_URL_end}' for i in range(len(searchresults)) if len(searchresults[i][str(i)]["summary"]) > 0])
...
def chatcompletion(system_prompt, prompt, completiontemperature, completionmaxtokens, assistantmessage = None, prompt2 = None):
    ...
    try:
        ...
        if assistantmessage and prompt2:
            ...
            #DEBUG OUTPUT
            debuglog("chatcompletion - Multi-Message:")
            prompt2_lines = split_lines(prompt2)
            formatted_prompt2 = "\n".join(f"<{line}>" for line in prompt2_lines)
            messages_DEBUG = [
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": prompt},
                {"role": "assistant", "content": assistantmessage},
                {"role": "user", "content": formatted_prompt2},
            ]
            for message in messages_DEBUG:
                debuglog(f"{message['role']}: {message['content']}")
            #END DEBUG OUTPUT
...
def split_lines(text, max_chars=180):
